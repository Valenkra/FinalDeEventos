...
export default class LocationsRepository {
    ...
    getLocationByProvinceId = async (id) => {
        ...
        try {
            await client.connect();
            const sql = `SELECT L.id, L.name, L.id_province, L.latitude, L.longitude FROM Locations L
    ...
    getLocationByEvenLocationId= async (id) => {
        let returnArray = null;
        const client = new Client(DBConfig);
        try {
            await client.connect();
            const sql = `SELECT L.id, L.name, L.latitude, L.longitude 
